...
class Paraphraser(nn.Module):
    ...
    def get_contrastive_loss(self, inputs, outputs=None):
        """
        ...
        with torch.no_grad():
            ...
            # Retrieve the best output
            pibleu_score = get_pibleu_score(input_ids, sequences, self.tokenizer) # batch_size * batch_size
            best_beam = torch.argmax(pibleu_score, dim=-1) # beam with highest score
            best_beam_idx = torch.tile(best_beam.unsqueeze(1).unsqueeze(2), (1, 1, sequences.size(2)))

            # Get boundaries and decoder_mask to obtain the shared prefix
            seq1 = sequences # all sequences(best sequence is masked out later), bacth_size *  batch_size
            seq2 = torch.ones_like(seq1) * torch.gather(sequences, 1, best_beam_idx) # best sequence tiled in size of seq1
            assert torch.equal(seq2[0, 0, :], sequences[0, best_beam[0], :])
        ...
        for i in range(batch_size):
            ...
            # mask out tok_dif for best sequence
            tok_dif *= (torch.arange(0, tok_dif.size(0), device=best_beam.device) != best_beam[i])
